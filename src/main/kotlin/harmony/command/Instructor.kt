...
open class Instructor(name: String) : Command(name.trim().lowercase()), Instructable {
    ...
    /**
     * The usage message of this instructable represented as an Adventure [Component].
     *
     * This message is automatically sent to the command sender when the number of
     * arguments provided exceeds [maxArgs], or when a usage failure is explicitly triggered.
     */
    override var componentUsageMessage: Component = Component.empty()

    /**
     * The message sent to the sender when permission is denied, represented as an Adventure [Component].
     *
     * If the sender does not have the required permission to execute this instructable,
     * this component will be displayed. If null, a default fallback message will be used.
     */
    override var componentPermissionMessage: Component =
        "<red>You don't has permission to run this command.".asComponent
    ...
    override fun execute(sender: CommandSender, name: String, args: Array<out String>): Boolean {
        ...
        if (!testPermissionSilent(sender)) {
            sender.sendMessage(this.componentPermissionMessage)
            return false
        }
        ...
        if (!this::executor.isInitialized || isHelpArg(firstArg) && childrens.isNotEmpty()) {
            val helpCommandPermission = this.helpPermission
            if (helpCommandPermission.isNullOrBlank() || sender.hasPermission(helpCommandPermission)) {
                if (sender is Player) {
    ...
    override fun testPermissionSilent(target: CommandSender): Boolean {
        val commandPermission = this.permission
        return if (commandPermission.isNullOrBlank()) true else target.hasPermission(commandPermission)
    }
    ...
    fun sendHelpToPlayer(player: Player) {
        ...
        player.sendMessage(" <aqua><bold>${name.uppercase()} COMANDOS</bold></aqua>".asComponent)
    ...
    fun sendHelpToConsole(sender: CommandSender) {
        ...
        sender.sendMessage(" <aqua><bold>${name.uppercase()} COMANDOS</bold></aqua>".asComponent)

        childrens.forEachIndexed { index, child ->
            if (child.showInHelp) {
                val prefix = if (index == childrens.size - 1) "┗" else "┃"
                sender.sendMessage(" <aqua><bold>$prefix</bold></aqua> <white>/${child.fullUsage}</white>".asComponent)
            }
        }
